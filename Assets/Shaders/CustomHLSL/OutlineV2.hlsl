...
void DepthSobel_float(float2 UV, float Thickness, out float Out) {
    ...
    [unroll] for (int i = 0; i < 9; i++) {
        float depth = SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture, sampler_CameraDepthTexture, UV + sobelSamplePoints[i] * Thickness);
...
//     float up = SampleDepth(uv + float2(0.0, 1.0) * Thickness);
//     float down = SampleDepth(uv + float2(0.0, -1.0) * Thickness);
//     float left = SampleDepth(uv + float2(1.0, 0.0) * Thickness);
//     float right = SampleDepth(uv + float2(-1.0, 0.0) * Thickness);
//     float centre = SampleDepth(uv);

//     float depth = max(max(up, down), max(left, right));
//     Out =  float2(clamp(up - centre, 0, 1) + clamp(down - centre, 0, 1) + clamp(left - centre, 0, 1) + clamp(right - centre, 0, 1), depth);
//     }
...
void ColorSobel_float(float2 UV, float Thickness, out float Out) {
    ...
    [unroll] for (int i = 0; i < 9; i++) {
        ...
        float3 rgb = SAMPLE_TEXTURE2D(_CameraColorTexture, sampler_CameraColorTexture,  UV + sobelSamplePoints[i] * Thickness);
